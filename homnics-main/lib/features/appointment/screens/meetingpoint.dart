...
class MeetingPoint extends StatefulWidget {
  Meeting meeting;
  MeetingPoint({required this.meeting, super.key});
  @override
  _MeetingPointState createState() => _MeetingPointState();
}

class _MeetingPointState extends State<MeetingPoint> {
  var serverText = TextEditingController();
  var roomText = TextEditingController(text: "omni_room_sample_1234");
  var subjectText = TextEditingController(text: "Subject1");
  var nameText = TextEditingController(text: "User1");
  var emailText = TextEditingController(text: "fake1@email.com");
  var iosAppBarRGBAColor =
  ...
  _joinMeeting() async {
    ...
    final options = JitsiMeetingOptions(
        token: widget.meeting.planBeneficiaryJwt,
        room: roomText.text,
        serverURL: serverUrl,
  ...
  void setUp() {
    String url = widget.meeting.meetingLink;
    roomText.text = url.split("/").last;
    serverText.text = url.substring(0, url.lastIndexOf(roomText.text));
    // serverText.text = widget.meeting.meetingLink;
    _joinMeeting();
    setState(() {});
  }
}
